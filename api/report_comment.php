...
.cmt-btn.reply-active { background: rgba(0,229,255,0.08); color: var(--accent); border: 1px solid rgba(0,229,255,0.15); }
...
.cmt-search-input {
    ...
    padding: 11px 16px 11px 40px; color: var(--text);
    font-family: 'Inter', sans-serif; font-size: 13px;
    transition: border-color 0.2s;
}
.cmt-search-input:focus { outline: none; border-color: var(--accent); }
.cmt-search-input::placeholder { color: var(--muted); opacity: 0.7; }
.cmt-search-icon {
...
.cmt-search-clear {
    ...
    cursor: pointer; display: none; line-height: 1; padding: 2px 4px;
}
.cmt-search-clear:hover { color: var(--text); }
.cmt-search-results {
    font-family: 'Space Mono', monospace; font-size: 10px;
    color: var(--muted); margin-bottom: 10px; min-height: 16px;
}
.cmt-highlight { background: rgba(0,229,255,0.2); border-radius: 2px; padding: 0 2px; }
...
<section class="section" id="comentarios">
  ...
  <div class="comments-section">
    ...
    <!-- Pesquisa -->
    <div class="cmt-search-wrap">
      <span class="cmt-search-icon">🔍</span>
      <input type="text" class="cmt-search-input" id="cmtSearch" placeholder="Pesquisar comentários…" oninput="onSearch(this.value)">
      <button class="cmt-search-clear" id="cmtSearchClear" onclick="clearSearch()">×</button>
    </div>
    <div class="cmt-search-results" id="cmtSearchResults"></div>
...
(function(){
  ...
  var allComments=[], currentFilter='all', currentSort='likes', currentSearch='', offset=0, LIMIT=20;
  ...
  window.onSearch = function(val) {
    ...
    var clearBtn = document.getElementById('cmtSearchClear');
    if (clearBtn) clearBtn.style.display = currentSearch ? 'block' : 'none';
    renderComments();
  };
  window.clearSearch = function() {
    var inp = document.getElementById('cmtSearch');
    if (inp) inp.value = '';
    currentSearch = '';
    var clearBtn = document.getElementById('cmtSearchClear');
    if (clearBtn) clearBtn.style.display = 'none';
    var resEl = document.getElementById('cmtSearchResults');
  ...
  function filterBySearch(list) {
    ...
    return list.filter(function(c) {
      var inContent  = (c.content||'').toLowerCase().indexOf(currentSearch) >= 0;
      var inAuthor   = (c.full_name||'').toLowerCase().indexOf(currentSearch) >= 0;
      var inUsername = (c.username||'').toLowerCase().indexOf(currentSearch) >= 0;
      var inReplies  = (c.replies||[]).some(function(r) {
        return (r.content||'').toLowerCase().indexOf(currentSearch) >= 0
            || (r.full_name||'').toLowerCase().indexOf(currentSearch) >= 0;
      });
      return inContent || inAuthor || inUsername || inReplies;
    });
  }
  ...
  function highlight(text, term) {
    ...
    return escaped.replace(new RegExp('(' + escapedTerm + ')', 'gi'), '<mark class="cmt-highlight">$1</mark>');
  }
  ...
  function renderComments() {
    ...
    var searched = filterBySearch(filtered);
    var sorted   = sortComments(searched);

    var resEl = document.getElementById('cmtSearchResults');
    if (resEl) {
      if (currentSearch) {
        resEl.textContent = sorted.length
          ? sorted.length + ' resultado' + (sorted.length!==1?'s':'') + ' para "' + currentSearch + '"'
    ...
    if (!sorted.length) {
      list.innerHTML='<div class="empty-state"><div class="empty-icon">💬</div><p>'
        +(currentSearch?'Nenhum comentário encontrado.':'Ainda não há comentários nesta categoria.')+'</p></div>';
      return;
    }
  ...
  function renderThread(c) {
    ...
    var likeBtn='<button class="cmt-btn'+(c.user_liked?' liked':'')+'" onclick="toggleLike('+id+',this)">'
      +likeIcon+' <span class="like-count">'+likeCnt+'</span></button>';

    // Botão responder — com contador se houver respostas
    // Um clique abre; dois cliques fecha (toggle)
    var replyLabel = repCount > 0 ? '💬 Responder (' + repCount + ')' : '💬 Responder';
    ...
    var reportBtn=(CUR_UID&&CUR_UID!==c.user_id)
      ?'<button class="cmt-btn report-btn" onclick="openReportModal('+id+')" style="margin-left:auto">🚩</button>':'';
    var modBtns=IS_MOD
    ...
    var contentHtml = currentSearch
      ? highlight(c.content, currentSearch)
      : esc(c.content).replace(/\n/g,'<br>');

    var repliesHtml = repCount > 0
      ? '<div class="cmt-replies-section" id="replies-'+id+'">'
      ...
      +'<div class="cmt-main">'
        +'<div class="cmt-author-row">'
          ...
          +'<div class="cmt-author-meta" style="flex:1;min-width:0">'
            +'<a class="name-link" href="perfil_publico.php?id='+c.user_id+'">'
              +(currentSearch ? highlight(c.full_name, currentSearch) : esc(c.full_name))
            +'</a>'
  ...
  function renderReplyItem(r) {
    ...
    var reportBtn=(CUR_UID&&CUR_UID!==r.user_id)
      ?'<button class="cmt-btn report-btn" onclick="openReportModal('+r.id+')" style="margin-left:auto">🚩</button>':'';

    var contentHtml = currentSearch
      ? highlight(r.content, currentSearch)
    ...
    return '<div class="cmt-reply-item" id="cmt-'+r.id+'">'
      +'<div class="cmt-author-row" style="margin-bottom:8px">'
        ...
        +'</div>'
      +'</div>'
      +'<div class="cmt-content cmt-content-sm">'+contentHtml+'</div>'
      +'<div class="cmt-actions">'
        +'<button class="cmt-btn'+(r.user_liked?' liked':'')+'" onclick="toggleLike('+r.id+',this)">'
  ...
  // ── Toggle área de resposta (corrigido) ───────────────────────
  // Estado guardado no DOM: rform tem classe "open" quando visível
  // Clicar sempre alterna — se aberto fecha, se fechado abre
  window.toggleReplyArea = function(id) {
    ...
    if (!isOpen) {
      // Abrir: mostrar respostas existentes + formulário
      form.classList.add('open');
      if (replies) replies.classList.add('open');
      if (btn) btn.classList.add('reply-active');
      var ta = form.querySelector('textarea');
      if (ta) { setTimeout(function(){ ta.focus(); }, 50); }
    } else {
      // Fechar: esconder tudo
      form.classList.remove('open');
      if (replies) replies.classList.remove('open');
      if (btn) btn.classList.remove('reply-active');
    }
  };

  // Utilizadores não logados — só ver/esconder respostas
